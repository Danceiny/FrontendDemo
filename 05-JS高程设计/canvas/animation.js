!(function () {
    ...
    function init(self) {
        ...
        if (self.mode === 'array') {
            ...
            //计算雪碧图数量
            self.imgsLen = Math.round(2 * self.imgs[0].width / (self.canvas.width * self.options.ratio))
        }
    ...
    function drawImg(self, n) {
        ...
        if (self.mode === 'array') {
            ...
            let img = self.imgs[0];
            var imgWidth = img.width / self.imgsLen;
            self.ctx.drawImage(img, imgWidth * n, 0, imgWidth, img.height, 0, 0, self.canvas.width, self.canvas.height);
        } else {
